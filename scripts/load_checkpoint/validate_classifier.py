...
def load_classifier(cfg_path, model_ckpt_path):
    ...
    cfg.datamodule.train_dataloader_conf.batch_size = 1
    val_dataloader = instantiate(cfg.datamodule).train_dataloader()
    ...
    with torch.cuda.amp.autocast():
        with torch.no_grad():
            for batch, labels in val_dataloader:
                ...
                batches.append((batch.cpu(), labels.cpu()))
    ...
    for i, (batch, labels) in enumerate(batches):
        print(f'batch_{i}')

        if i == 4:
            # plt.imsave(f'imgs/batch_{i}/embeddings.png', batch[0,0])
            plt.imsave(f'imgs/batch_{i}/labels_{i}.png', labels[0])
        #     conti
        ...
        #         img[mask] = 1
        #
        #         imsave_path = Path(f'imgs/batch_{i}/embedding_{j}.png')
...
if __name__ == '__main__':
    ...
    os.environ["CAMELYON16_EMBEDDINGS_PATH"] = "/home/robertsc/2D-VQ-AE-2/vq_ae/multirun/2022-05-24/10-19-21/0/encodings_incomplete.hdf5"
